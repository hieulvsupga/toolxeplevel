...
/**
 * Giữ-kéo chuột trái: tô vùng chữ nhật ngang (4 hướng X/Z) ở tầng neo.
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (e.ctrlKey) {
        ...
        const yAtMouse = Math.floor(C.y + sc);
        if (yAtMouse >= d.baseY) {
          useDrag.getState().setLayers(d.baseY, Math.min(yAtMouse, d.baseY + MAX_SIDE - 1));
        } else {
          useDrag.getState().setLayers(Math.max(yAtMouse, d.baseY - MAX_SIDE + 1), d.baseY);
        }
        return;
      }

      // ----- MẶT ĐÁY: raycast lên mặt phẳng ngang giữa tầng neo -----
      plane.set(UP, -(d.baseY + 0.5));
      ...
      useDrag.getState().move(Math.floor(hit.x), Math.floor(hit.z));
    };
