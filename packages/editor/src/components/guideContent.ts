...
export const GUIDE: GuideSection[] = [
  ...
  {
    id: 'lopboc',
    title: 'Lớp bọc 🧊 Băng / 🟪 Khối lớn — bọc một hộp khối lại',
    blocks: [
      {
        kind: 'p',
        text: 'Lớp bọc là một hộp có hp: băng là cái VỎ trùm lên khối có sẵn (phá vỏ mới bắn được khối bên trong), còn khối lớn tự nó là một CỤC ĐẶC thay cho cả cụm nhỏ — đặt lên chỗ trống cũng được (data thật có level 0 khối thường mà 64 khối lớn). Có hai cách dựng. (1) ĐẶT NHƯ KHỐI — nhanh khi cần rải nhiều: bấm nút 🧊 hoặc 🟪 ở nhóm công cụ trên toolbar (phím W về đúng loại vừa chọn), rồi bấm vào scene phát nào ra hộp phát đó; ô đang trỏ là góc nhỏ nhất của hộp nên xếp cạnh nhau là khít. Điểm tựa để bấm có thể là mặt sàn, mặt một khối thường, HOẶC MẶT CỦA MỘT LỚP BỌC ĐÃ CÓ — nên xếp chồng khối lớn lên nhau được, không cần khối thường làm nền. hp và màu sửa ngay trên dòng trong bảng 🧊 góc dưới-trái. GIỮ VÀ KÉO thì hộp DÀI RA theo hướng kéo — một cục to dần chứ không phải nhiều hộp như công cụ đặt khối — kéo trong mặt phẳng của chính mặt vừa bấm (bấm sàn thì kéo ngang theo X/Y, bấm sườn khối thì kéo trong mặt đứng). Giữ thêm Ctrl giữa chừng thì chuột chuyển sang kéo CHIỀU VUÔNG GÓC mặt đó: đang kéo bề ngang thì Ctrl ra chiều cao (lên hoặc xuống), đang kéo trong mặt đứng thì Ctrl ra bề sâu; thả Ctrl là quay lại hai chiều cũ, phần vừa kéo được giữ nguyên. Chiều nào chưa đụng tới thì rộng 1 ô. Khung xem trước đỏ nghĩa là chỗ đó đè lên lớp bọc khác, Esc huỷ cú kéo. (2) BỌC CỤM ĐANG CHỌN — hợp khi bọc đúng một cụm có sẵn, và là cách DUY NHẤT dựng được lớp bọc không phải hình hộp: quét vùng bằng ⬚ Chọn hoặc ▭ Chọn 2D, chọn kiểu ở cặp nút ⬚ Bám khối / ⬛ Cả hộp, rồi bấm 🧊 Băng hoặc 🟪 Khối lớn trong bảng vùng chọn. BÁM KHỐI (mặc định) = lớp bọc ôm đúng những ô ĐANG CÓ KHỐI, ô trống trong vùng không tính — dựng được quả cầu, vỏ rỗng, hình chữ L, đúng cách data thật của game làm (Level_79 là khối lớn hình cầu 416 ô trong hộp bao 8×8×8; Level_393 là vỏ băng rỗng 772 ô trong hộp 12×13×12). Với khối lớn thì mấy ô trong hộp bao mà lớp bọc KHÔNG chiếm vẫn là khối thường bình thường, vẫn nằm trong layers — đúng như data game (Level_79 giữ 72 khối như vậy). CẢ HỘP = lấy trọn hộp chọn kể cả ô trống, tức hộp đặc như trước. Vùng chọn lấp kín hộp thì hai kiểu cho ra y hệt nhau. Hai loại dùng chung một khuôn data (bounds + hp + hpTexts + innerVoxelPositions), khác hai chỗ: ghi vào iceWrapperData hay largeVoxelData, và khối lớn có thêm colorType (màu của cục lớn, mặc định lấy màu áp đảo trong hộp, sửa bằng ô chọn màu ngay trên dòng). Bấm nhầm loại thì đổi tại chỗ bằng icon ở đầu dòng.',
      },
      {
        kind: 'p',
        text: 'Danh sách 🧊 ở góc dưới-trái liệt kê mọi lớp bọc: cỡ hộp bao (lớp bọc có hình riêng thì ghi thêm ·số ô, ví dụ 8×8×8·416 — trỏ vào để xem đầy đủ), số khối bên trong, ô nhập hp, nút ⬚ chọn lại hộp đó, nút ☰ mở bảng liệt kê từng ô, và 🗑 bỏ lớp bọc (không xoá khối). BẢNG ☰ là chỗ sửa hình của lớp bọc tới từng ô: liệt kê toàn bộ ô trong hộp bao kèm màu khối ở ô đó (xếp x → y → z, đúng thứ tự ghi ra file); bỏ dấu tích ở ô nào là gỡ ô đó ra khỏi lớp bọc, tích lại là cho vào — bỏ nhầm vẫn lùi được. Ô tích “chỉ ô có khối” ẩn các ô trống đi cho danh sách gọn. Trỏ vào một dòng thì ô đó sáng trắng trong scene. Nút “👁 chỉ hiện lớp này” ẩn hẳn mọi khối KHÔNG thuộc lớp bọc, chỉ để lại khối của nó — lúc này bấm vào một khối trong scene thì bảng tự cuộn tới đúng dòng của ô đó và tô sáng lên, khỏi phải dò toạ độ; chuột trái lúc đó là chọn ô chứ không đặt/xoá khối nữa. Đóng bảng là scene trở lại bình thường. Đây chỉ là chuyện xem, không đụng gì tới data. Hai nút tắt: “bỏ n ô trống” gỡ hết ô không có khối (biến hộp đặc thành lớp bọc ôm đúng hình khối đang có), “cả hộp” cho trùm lại trọn hộp bao. Gỡ ô KHÔNG xoá khối: với khối lớn thì khối ở ô đó quay về làm khối thường (lại nằm trong layers, lại tốn đạn), với băng thì ô đó hết vỏ. Hộp bao co theo: gỡ hết một lát ngoài rìa là hộp tự thu lại, bounds ghi ra file đổi theo. Trỏ chuột vào một dòng là lớp bọc đó SÁNG TRẮNG trong scene kèm tô sáng từng khối nó bọc (các lớp khác mờ đi); bấm vào dòng để giữ sáng cố định, bấm lại để thôi. Nút ✕ ở góc bảng ẩn hẳn bảng đi; bật lại bằng nút 🧊 trên toolbar (nút đó cũng hiện số lớp bọc, và xám khi level chưa có lớp nào). Trong scene mỗi lớp bọc là một khung xanh băng, vẽ xuyên khối để không bị cụm che mất.',
      },
      {
        kind: 'ul',
        items: [
          'Bạn chỉ nhập HỘP và HP (khối lớn thêm màu). Còn lại tool tự tính lúc xuất, theo đúng data thật của game (đối chiếu với 611 lớp bọc trong Data/DataExample2): bounds = tâm + extent NỬA cỡ tính cả nửa ô hai đầu (khớp 611/611), innerVoxelPositions = các ô lớp bọc CHIẾM — hộp đặc thì là toàn bộ ô của hộp kể cả ô trống (608/611 lớp bọc trong data thật đúng như vậy), còn lớp bọc bám khối thì là đúng hình của nó (3 ca còn lại trong data thật là hình có chủ ý: quả cầu, vỏ rỗng), hpTexts dán lên cặp mặt vuông góc trục mỏng nhất — hộp vuông thì bốn mặt bên — nhô ra khỏi mặt 0.1 (vị trí khớp 611/611).',
          'Sửa hộp: kéo vùng chọn mới rồi bấm lại, xoá lớp cũ bằng 🗑. Khung trong scene đổi màu theo loại: băng xanh băng, còn khối lớn vẽ bằng ĐÚNG màu game của nó nên nhìn khung là biết cục lớn sắp ra màu gì; lớp đang soi thì sáng trắng.',
          'Chia nhỏ khối (⧉) cũng nhân hộp bọc theo, nên lớp bọc vẫn trùm đúng cụm cũ.',
          'KHÁC NHAU QUAN TRỌNG giữa hai loại: khối lớn là một cục THAY cho cụm nhỏ, nên lúc xuất các voxel trong hộp bị gỡ khỏi layers (đúng như data game: 536/536 khối lớn không có ô nào trong layers). Lớp băng thì ngược lại, voxel bên trong vẫn nằm nguyên trong layers (75/75).',
          'ĐẠN: mỗi khối lớn tốn đúng hp viên theo MÀU CỦA NÓ, chứ không phải một viên cho mỗi voxel nó nuốt. Đo trên data thật: “đạn − khối trong layers” bằng đúng tổng hp của khối lớn cùng màu ở 35/36 level. Bảng Blaster, phần tạo súng tự động và phần soát lúc xuất đều đã đếm theo luật này — nên đổi hp của khối lớn là bảng cân đối đạn đổi theo.',
          'Xuất/nhập giữ nguyên lớp bọc cả hai loại, kể cả hình riêng (đối chiếu 134 khối dữ liệu lớp bọc của 67 level trong Data/DataExample2: đọc vào ghi ra trùng khít từng ô) — nhập lại file vừa xuất là ra đúng level cũ (trước đây tool báo “iceWrapperData / largeVoxelData sẽ mất nếu xuất đè”, giờ không còn).',
        ],
      },
      {
        kind: 'p',
        text: 'Bảng Xuất soát sẵn các lỗi không nhìn thấy bằng mắt: hộp không bọc khối nào, hp ≤ 0, hộp chỉ bọc tường, vùng bọc có ô trống bên trong (vào game mấy ô đó là vỏ băng rỗng), bị che kín cả 6 mặt (không dán được số HP), và hai lớp bọc CHỒNG NHAU (một voxel không thể thuộc hai lớp bọc).',
      },
      {
        kind: 'note',
        text: 'Nút “Thử giải” CHƯA biết luật lớp bọc — nó coi như khối bên trong bắn được ngay, nên màn thật khó hơn điểm nó chấm (kết quả có ghi lại lưu ý này khi level có lớp bọc). Và nhớ: LevelData.cs chỉ ghi iceWrapperData là “V1-relevant”, còn largeVoxelData nằm trong nhóm “extensibility placeholders” — tool xuất khối lớn đúng chuẩn (kể cả thứ tự field: colorType đứng trước bounds như C# khai báo) nhưng gameplay V1 chưa đọc, nên trong game chưa thấy tác dụng (bảng Xuất cũng nhắc lại khi level có khối lớn).',
      },
    ],
  },
